...
chem_data<-read.csv("/Users/Meltem/Downloads/drug_desc_to_wale.txt",sep="\t",header = T)
head(chem_data)
str(chem_data)
dim(chem_data)
...
chem_data[,5:150]<-lapply(chem_data[,5:150],as.numeric) # Convert the chemical properties columns into numeric 
...
sapply(chemdataselected[,5:146],var)
...
lm_model <- train(score ~ ., data = train_data[,c("score",colnames(chemdataselected[,5:146]))], # dot means all variables, tilde defines relationships with variables
                  method = "lm",
                  trControl = trainControl(method = "cv", number = 5)
                  )
rf_model <- train(score ~ ., data = train_data[,c("score",colnames(chemdataselected[,5:146]))], # dot means all variables, tilde defines relationships with variables
                  method = "rf",
                  trControl = trainControl(method = "cv", number = 5),
...
pre_ft_selection_lm<-write.csv(lm_importance$importance,"pre_ft_sel_lm.txt")
dim(lm_importance$importance)
...
pre_ft_selection_rf<-write.csv(rf_importance$importance,"pre_ft_sel_rf.txt")
...
rfe_result <- rfe(x = chemdataselected[, 5:146], y = chemdataselected$score, sizes = c(5,10,15), rfeControl = control)
...
print(paste("Linear regression MAE:", round(lm_mae, 2)))
...
plot(lm_importance, top = 10)
post_ft_selection_lm<-write.csv(lm_importance$importance,"post_ft_sel_lm.txt")
...
rfe_result_rf <- rfe(x = chemdataselected[, 5:146], y = chemdataselected$score, sizes = c(5), rfeControl = control)
...
print(paste("Random forest regression MAE:", round(rf_mae, 2)))
...
plot(rf_importance, top = 10)
post_ft_selection_rf<-write.csv(rf_importance$importance,"post_ft_sel_rf.txt")
...
ggplot(results, aes(x = actual_score)) +
  ...
  geom_point(aes(y = Predicted_RF, color = "Random Forest")) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") + # adds a reference line (y=x)
  labs(title = "Predicted vs. Actual Score after RFE",
       x = "Actual Score",
       y = "Predicted Score") + theme_minimal()+xlim(c(min(lm_predictions), max(rf_predictions)))
...
d_results<-data.frame(reg_method=c("Linear regression","Random forest regression"),
                      actual_score=test_data$score,
                      regressions=c(mean(lm_predictions),mean(rf_predictions)))
ggplot(d_results,aes(x=actual_score))+ geom_density(aes(fill=reg_method))+geom_vline(xintercept=d_results$regressions)+labs(title = "Density Plot of Predicted vs. Actual Score after RFE",x = "Actual Score",y = "Density") + theme_minimal()+xlim(range(test_data$score))
